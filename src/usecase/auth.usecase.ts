...
export class AuthUsecase implements IAuthUsecase {
  ...
  async userSigninGoogle(data: string) {
    ...
    const user = await this.authRepository.emailAuth(payload.email);
    if (!user) {

      const data: IUser = { email: payload.email, userid: payload.email,password:null } as unknown as IUser
      const newUser = await this.authRepository.create(data,true);

    }
      const data1:IAuth = {email:payload.email} as IAuth

      const payloadJWT = { id: user._id, email: user.email, user_id: user.userid };
      const token = signJWT(payloadJWT, 8);
  
      return await this.authRepository.login(data1, token);

...
class TripRepository {
    findByIds(tripIds) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!(tripIds === null || tripIds === void 0 ? void 0 : tripIds.length))
                return [];
            const objectIds = tripIds
                .filter(Boolean)
                .map((id) => new mongoose_1.default.Types.ObjectId(id));
            const trips = yield trip_model_1.TripModel.find({ _id: { $in: objectIds } });
            const byId = new Map(trips.map((t) => [String(t._id), t]));
            // Preserve semantic rank ordering
            return tripIds.map((id) => byId.get(String(id))).filter(Boolean);
        });
    }
